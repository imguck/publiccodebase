...
int StructureManager::placeStructureFromDeed(CreatureObject* creature, StructureDeed* deed, float x, float y, int angle) {
	...
	/*if (deed->getFaction() != 0){
		String areaPath = "object/gcwbase_area.iff";
		info("Faction base placed, placing Active Area", true);
		ManagedReference<GCWBaseActiveArea*> gcwArea =
				(zoneServer->createObject(areaPath.hashCode(), 1)).castTo< GCWBaseActiveArea*>();
	}*/
