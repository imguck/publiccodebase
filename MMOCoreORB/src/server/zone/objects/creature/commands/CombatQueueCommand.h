	...
	int doCombatAction(CreatureObject* creature, const uint64& target, const UnicodeString& arguments = "", ManagedReference<WeaponObject*> weapon = NULL) const {
		...
		if (creature->isPlayerCreature() && targetObject->isPlayerCreature()) {
			if (!combatManager->areInDuel(creature, targetObject.castTo<CreatureObject*>())) {
				shouldTef = true;
				PlayerObject* targetGhost = targetObject.castTo<CreatureObject*>()->getPlayerObject().get();
				WeaponObject* weap = targetObject.castTo<CreatureObject*>()->getWeapon();

				if ((targetGhost != NULL && weap != NULL &&((targetGhost->isJedi() && weap->isJediWeapon()))) || targetGhost->hasJediTef()){
					shouldJediTef = true;
					//if (targetGhost->isJedi())
						targetGhost->updateLastJediCombatActionTimestamp();
				}
			}
			...
			if (targetOwner != NULL && targetOwner->isPlayerCreature()) {
				if (!combatManager->areInDuel(creature, targetOwner)) {
					shouldTef = true;
					PlayerObject* ownerJedi = targetOwner->getPlayerObject();
					WeaponObject* ownerWeap = targetOwner->getWeapon();

					if (ownerJedi != NULL && ownerWeap != NULL){
						if ((ownerJedi->isJedi() && ownerWeap->isJediWeapon()) || ownerJedi->hasJediTef()){
							shouldJediTef = true;
							if (ownerJedi->isJedi())
								ownerJedi->updateLastJediCombatActionTimestamp();
						}
					}
				}
			}
		} else if (creature->isPlayerCreature() && targetTano->getFaction() != 0 && targetTano->getFaction() != creature->getFaction()){
			shouldTef = true;
		}
		...
		// Update Jedi TEF Duration
		if (shouldJediTef && creature->isPlayerCreature()) {
			PlayerObject* ghost = creature->getPlayerObject().get();

			if (ghost != NULL) {
				ghost->updateLastJediCombatActionTimestamp();
			}
		} else if (shouldJediTef && creature->isPet()) {
			ManagedReference<CreatureObject*> owner = creature->getLinkedCreature().get();
			...
			if (owner != NULL && owner->isPlayerCreature()) {
				...
				if (ownerGhost != NULL) {
					Locker olocker(owner, creature);
					ownerGhost->updateLastJediCombatActionTimestamp();
				}
			}
		}
