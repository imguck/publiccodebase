...
bool CreatureObjectImplementation::isAggressiveTo(CreatureObject* object) {
	...
	if (ghost->hasJediTef()) 
		return true;
	//if (ghost->hasPvpTef() && (getFaction() != object->getFaction() || object->getFaction() != 0) )
	//	return true;
	...
	/*if (object->getPvpStatusBitmask() & CreatureFlag::TEF && (getFaction() != object-> getFaction() || object->getFaction() != 0)){
		return true;
	}*/
...
bool CreatureObjectImplementation::isAttackableBy(TangibleObject* object, bool bypassDeadCheck) {
	...
	//if((object->getPvpStatusBitmask() & CreatureFlag::OVERT) && (!(getPvpStatusBitmask() & CreatureFlag::OVERT) || !(getPvpStatusBitmask() & CreatureFlag::TEF)))
	//	return false;
...
bool CreatureObjectImplementation::isAttackableBy(CreatureObject* object, bool bypassDeadCheck) {
	...
	/*if (ghost->isJedi() && pvpStatusBitmask & CreatureFlag::TEF){
		info(getFirstName() + " is a Jedi", true);
		return true;
	}*/
	if (getFaction() == 0 || object->getFaction() == 0)
		return false;

	if ((pvpStatusBitmask & CreatureFlag::TEF) && (object->getFaction() != getFaction()) && (object->getFaction() != 0)) {
		return true;
	}
	//if (ghost->isJedi() && isAttackableBy(object) && object->)
...
bool CreatureObjectImplementation::isHealableBy(CreatureObject* object) {
	...
	if (ghost != NULL && playerGhost != NULL){
		if ((ghost->hasPvpTef() || playerGhost->hasPvpTef()) && (getFaction() != object->getFaction()) && (!ghost->hasJediTef() && !playerGhost->hasJediTef()))
			return false;
	}
	...
	if (ghost != NULL){
		if (playerGhost != NULL && playerGhost->hasJediTef()){
