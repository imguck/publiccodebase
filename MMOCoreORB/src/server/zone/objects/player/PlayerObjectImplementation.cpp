...
#include "server/zone/objects/player/events/JediTefRemovalTask.h"
...
void PlayerObjectImplementation::notifyOnline() {
	...
	//playerCreature->sendExecuteConsoleCommand("/chatRoom join SWG.swgemudev.General");
	//playerCreature->sendExecuteConsoleCommand("/chatRoom join SWG.swgemudev.PvP");
}
...
Time PlayerObjectImplementation::getLastJediCombatActionTimestamp() {
	return lastJediCombatActionTimestamp;
}

void PlayerObjectImplementation::updateLastJediCombatActionTimestamp() {
	ManagedReference<CreatureObject*> parent = getParent().get().castTo<CreatureObject*>();

	if (parent == NULL)
		return;

	bool alreadyHasTef = hasJediTef();

	lastJediCombatActionTimestamp.updateToCurrentTime();
	lastJediCombatActionTimestamp.addMiliTime(60000); // 1 minute
...
bool PlayerObjectImplementation::hasJediTef() {
	return !lastJediCombatActionTimestamp.isPast();
}

void PlayerObjectImplementation::scheduleJediTefRemovalTask(bool removeNow) {
	ManagedReference<CreatureObject*> parent = getParent().get().castTo<CreatureObject*>();
	...
	if (jediTefTask == NULL) {
		jediTefTask = new JediTefRemovalTask(parent);
	}

	if (removeNow) {
		lastJediCombatActionTimestamp.updateToCurrentTime();

		if (jediTefTask->isScheduled()) {
			jediTefTask->cancel();
		}

		jediTefTask->execute();

	} else if (!jediTefTask->isScheduled()) {
		if (hasJediTef()) {
			jediTefTask->schedule(llabs(getLastJediCombatActionTimestamp().miliDifference()));
		} else {
			jediTefTask->execute();
		}
	}
